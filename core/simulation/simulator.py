...
class MonteCarloSimulator:
    ...
    def _load_upcoming_fixtures_directly(filepath):
        ...
        try:
            ...
            # Normalize any full/ESPN names to canonical football-data.co.uk names.
            df['HomeTeam'] = df['HomeTeam'].map(lambda t: TEAM_NAME_MAP.get(t, t))
            df['AwayTeam'] = df['AwayTeam'].map(lambda t: TEAM_NAME_MAP.get(t, t))
            ...
            # Drop rows with missing or blank team names
            df = df.dropna(subset=['HomeTeam', 'AwayTeam'])
            df = df[df['HomeTeam'].str.strip() != '']
            df = df[df['AwayTeam'].str.strip() != '']
            if df.empty:
                raise ValueError("No Allsvenskan fixtures found in upcoming fixtures file")
