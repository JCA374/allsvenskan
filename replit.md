...
- July 04, 2025. ENHANCED TEAM STRENGTH CALCULATIONS: Implemented fix.md improvements with comprehensive odds-based strength extraction system. Added OddsStrengthExtractor class that reverse-engineers team attack/defense strengths from betting odds using optimization algorithms. Integrated AdaptiveStrengthCalculator with dynamic window selection for optimal team performance analysis. Enhanced TeamStrengthCalculator with odds integration capabilities, supporting both historical-only and odds-enhanced calculation modes. Added sophisticated lambda optimization that matches Poisson model predictions to market odds using differential evolution algorithms. Enhanced Model Training page with options for historical vs odds-enhanced calculations, live API integration, and comparative analysis displays. System now extracts expected goals directly from betting markets to improve prediction accuracy.
- July 20, 2025. CRITICAL FIX: COLUMN NAME STANDARDIZATION: Implemented comprehensive solution from fix.md to resolve data merge failures causing missing fixtures in predictions. Root cause was inconsistent column naming conventions (Home_Team/Away_Team vs HomeTeam/AwayTeam vs home_team/away_team) across the data pipeline. Created ColumnStandardizer utility class with STANDARD_COLUMNS mapping and standardize_columns() method. Updated app.py, simulator.py to use standardized column names consistently. Created migration script (scripts/migrate_column_names.py) that converted all CSV files to use HomeTeam/AwayTeam standard (with automatic backups). Added comprehensive test suite (tests/test_column_standardization.py) to verify merge operations work correctly. System now prevents data loss during fixture prediction merge operations and maintains consistency across all data processing components.
```
